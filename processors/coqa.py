    ...
    def _create_examples(self, input_data, history_len,dataset_type = None):
        ...
        for i in range(len(datum['questions'])):
            ...
            example = CoqaExample(
                ...
                start_position = _qas['answer_span'][0] if dataset_type is None else 0, 
                end_position = _qas['answer_span'][1] if dataset_type is None else 0,
                rational_start_position = r_start if dataset_type is None else 0,
                rational_end_position = r_end if dataset_type is None else 0,
                additional_answers=_qas['additional_answers'] if 'additional_answers' in _qas else None,
            )
            if len(doc_tok) > 0:
                examples.append(example)
            #print("Words ",_datum['annotated_context']['word'],len(_datum['annotated_context']['word']))
            #print("Truncated ",doc_tok,len(doc_tok))
            #print(r_start,r_end)
        return examples
